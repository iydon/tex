...
def ping_links(*links, seconds=3, verify=False, **kwargs):
	...
	for link in tqdm.tqdm(links, **tqdm_config):
		try:
			response = requests.head(link,
				verify=verify, timeout=seconds, **kwargs)
...
def test_link_available():
	available_ext = ('.md', )
	ref_pattern = re.compile(r'https?://[^\)\s]+')
...
if __name__ == '__main__':
	...
	for func in tqdm.tqdm(filter(is_test, _locals_key), **tqdm_config):
		_func = _locals[func]
